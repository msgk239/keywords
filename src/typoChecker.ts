    ...
    public checkDocument(document: vscode.TextDocument): TypoItem[] {
        ...
        for (const typo in typoMap) {
            try {
                ...
                // 对于英文单词，使用词边界匹配
                if (isEnglishWord(typo)) {
                    regex = new RegExp(`\\b${escapedTypo}\\b`, 'g');
                    console.log(`使用词边界匹配英文单词: ${typo}`);
                } else {
                    // 对于中文或其他非英文单词，使用普通匹配
                    regex = new RegExp(escapedTypo, 'g');
                }
